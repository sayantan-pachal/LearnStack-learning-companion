...
function ResetPassword() {
    ...
    useEffect(() => {
        // ⬇️ CHANGE THIS LINE to "learnstack_user"
        const sessionData = localStorage.getItem("learnstack_user");
    ...
    const handleReset = async (e) => {
        ...
        setLoading(true);
        try {
            // Pass the email from session along with the passwords
            await account.updatePassword(userEmail, oldPassword, newPassword);
            showToast("Password updated successfully 🔐", "success");
            navigate("/dashboard", { replace: true });
        } catch (error) {
            console.error("Password Update Error:", error);
            showToast(error.message || "Failed to update password. Check your old password.", "error");
